...
No existing model supports a defensible probability that anthropogenic climate change will cause 1 billion excess deaths within a rolling year. The absence of such an estimate is not evidence that the threshold is impossible. It reflects a missing model class. This paper specifies 1177-G, a stochastic hybrid causal model of climate-triggered failure across crop, food, trade, energy, water, finance, governance, conflict, health, and displacement networks. The outcome is the maximum 365-day sum of excess mortality, with deaths assigned through competing hazards rather than added across sectoral studies. Buffer depletion, recovery time, behavioral response, and state capacity determine whether shocks dissipate or compound. A state-dependent cascade matrix measures amplification between layer-region failures, while minimal cut sets identify combinations capable of crossing the mortality threshold. The design combines historical calibration, survival-case validation, rare-event sampling, and paired factual and counterfactual climate ensembles. Existing crop, network, famine, and nuclear-winter studies constrain individual modules but cannot validate the global tail as a whole. A simulated billion-death year would therefore be a conditional model result rather than a forecast. The framework identifies the evidence and failure tests required before such a result could inform claims about climate risk.

## A Threshold Without a Probability

A death toll of 1 billion in 365 days is a demographic event before it is a rhetorical one. The United Nations estimated a world population of 8.2 billion in 2024 (United Nations, 2024). Against that reference population, the threshold is about 12.2%. It is close to the 10% mortality anchor used in one recent framework for global catastrophic risk (Arnscheidt et al., 2025). Crossing it would require a disturbance operating across regions and causes of death at a scale outside the range of modern climate-impact observations.
...
Calling that route constructible does not make it probable. It sets a modeling burden. The model must represent the systems that separate a physical hazard from death, and it must allow those systems to work. Grain stocks, substitution, imports, fiscal transfers, public health, migration, and international assistance prevent many shocks from becoming famines. A catastrophe model calibrated only on failure would confuse omission of resilience with evidence of collapse.

The purpose of 1177-G is to state this burden in executable terms. The name points to a historical systems-collapse analogy, while the suffix marks a global model. The paper does not ship that model. A credible implementation would be a multi-institution program with restricted microdata, sectoral simulators, historical reconstruction, and repeated external review. What can be supplied here is the causal specification, the validation ladder, and the conditions under which a claimed result should be refused.

## What the Existing Models Constrain
...
Hultgren et al. (2025) estimate climate effects across 12,658 regions and six staple crops while accounting for observed adaptation, with an aggregate response corresponding to an annual loss of about $5.5\times10^{14}$ kilocalories for each degree of warming, or around 120 kilocalories per person per day at the global scale. This production result cannot be converted directly into deaths because food reaches people through stocks, feed use, processing, prices, incomes, public distribution, and household allocation.

Model spread remains consequential even at moderate mean warming. Bevacqua et al. (2026) show that some modeled extreme outcomes at $2\,^{\circ}\mathrm{C}$ exceed multimodel-average outcomes at $4\,^{\circ}\mathrm{C}$ for selected metrics, including a breadbasket-drought measure. Their result argues for structural ensembles and plausibility checks. It does not license selection of the most destructive model member as the representative future.
...
The analogy should therefore guide questions about timing and overlap. Did a second shock arrive before the system recovered from the first? Which dependencies turned a local loss into a regional one? Which buffers failed, and which institutions restored them? It should not supply coefficients. Naming the model 1177-G is a reminder that collapse is a sequence of coupled losses. It makes no claim that history repeats its numerical form.

## States, Layers, and Clocks
...
where $z(t)$ contains exogenous and climate-forced hazards, $u(t)$ contains policy and behavioral actions, $\theta$ contains estimated parameters, and $\varepsilon(t)$ contains process error. Some transitions are continuous, such as depletion of a reserve. Others are discrete, such as port closure, export prohibition, government loss of territorial access, or emergency foreign assistance. Hence the hybrid form.
...
## Food, Trade, and Strategic Response
...
Finance enters through import capacity, exchange rates, sovereign borrowing, household income, and payment infrastructure. A food deficit causes mortality only after these access channels and relief responses are represented. This is where a biophysical crop model becomes a political-economy model.
...
is a local amplification diagnostic. Values above one indicate that a marginal failure generates more than one downstream failure under the current state and threshold definitions. $\mathcal{R}_C$ is not a universal reproduction number. $K$ changes with season, buffers, behavior, geography, and the size of the perturbation. Nonlinear cascades can occur below a local linear threshold after a large shock, while rapid repair can stop propagation above it.
...
## Calibration Against Failure and Survival
...
Temporal validation is essential. Parameters estimated through year $t$ should predict held-out episodes after $t$. Geographic transfer should be tested separately because the same observed indicator can encode different institutions. Latent states such as government capacity require measurement models with multiple indicators and uncertainty, rather than hand-assigned scores chosen after outcomes are known.
...
Rare-event algorithms create their own danger. If proposal distributions favor physically incoherent hazard combinations, they can populate the tail with impossible worlds. Every sampled driver must respect joint climate physics, demographic conservation, response constraints, and the calibrated dependence structure. Analysts should publish the dominant paths and importance weights alongside the final estimate.

The first scientific output may be a non-reachability result over validated parameter ranges. If no admissible combination crosses the threshold without leaving those ranges, the model has learned something useful. Parameters should not be widened simply to force a dramatic event.

## Attributing the Tail to Anthropogenic Climate Change
...
The billion-death question can then be stated without spectacle. It asks whether calibrated climate hazards can push a buffered, adaptive, politically responsive world into a region of state space where cross-layer failures outrun repair and produce 1 billion unique excess deaths in some 365-day window. Current evidence does not answer it. A model built to answer it must remain capable of saying no.
